...
bool compress_image_to_jpeg_file(const char *pFilename, int width, int height, int num_channels, const uint8 *pImage_data, const params &comp_params)
{
  ...
  if (!dst_image.init(&dst_stream, width, abs(height), num_channels, comp_params))
    return false;

  const int pitch = height < 0 ? -width*num_channels : width*num_channels;
  const uint8* pSrc = 0;
  if(pitch < 0)
  {
    pSrc = pImage_data + -pitch*(abs(height)-1); // start at "far end" of image
  }
  else
  {
    pSrc = pImage_data; // start at start of image
  }
  for (uint pass_index = 0; pass_index < dst_image.get_total_passes(); pass_index++)
  {
    for (int i = 0; i < abs(height); i++)
    {
       const uint8* pBuf = pSrc;
       pSrc += pitch;
